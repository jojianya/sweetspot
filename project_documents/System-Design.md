...
GET    /pins?bbox=lat1,lng1,lat2,lng2&category=food   — [public] pins within viewport, optional category filter
GET    /pins/:id                 — [public]
GET    /categories               — [public] fixed list of categories for the create-pin UI
POST   /pins                     — [auth required] create a pin (photo + lat/lng + caption + category)
...
CREATE TABLE pins (
    ...
    caption TEXT,
    category_id INT REFERENCES categories(id),  -- fixed category, required
    created_at TIMESTAMPTZ DEFAULT now()
);
...
-- photo_url points to local filesystem in dev, Cloudflare R2 in production (see build order)
-- migration order: 0001 extensions, 0002 users, 0003 categories+pins, 0004 reports, 0005 streams
```
...
Viewer count
  ...
  → Go API updates count, pushes via WebSocket to the stream's room
```
